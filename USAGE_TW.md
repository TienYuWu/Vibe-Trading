...
| run 變成 `failed`，訊息是 `run stalled: no LLM completion ... for 18xxs` | 看門狗比 LLM 重試預算早到。`VIBE_TRADING_LLM_TIMEOUT_SECONDS × (1 + MAX_RETRIES)` 必須小於 `VIBE_TRADING_RUN_STALL_TIMEOUT_SECONDS`。900×3=2700 > 預設 1800，所以要設 3600 |
